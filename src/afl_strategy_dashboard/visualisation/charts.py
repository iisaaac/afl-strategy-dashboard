...
def interstate_travel_count_chart(travel_counts: pd.DataFrame) -> go.Figure:
    ...
    chart_data = travel_counts.sort_values("interstate_travel_games", ascending=True)
    fig = px.bar(
        chart_data,
    ...
    _highlight_leader(fig, len(chart_data), TEAL)
...
def fixture_attractiveness_leaderboard_chart(ranked: pd.DataFrame) -> go.Figure:
    ...
    chart_data = chart_data.sort_values("fixture_attractiveness_score", ascending=True)
    fig = px.bar(
        chart_data,
    ...
    _highlight_leader(fig, len(chart_data), GREY)
...
def fan_growth_opportunity_leaderboard_chart(opportunities: pd.DataFrame) -> go.Figure:
    ...
    chart_data = chart_data.sort_values("fan_growth_opportunity_score", ascending=True)
    fig = px.bar(
        chart_data,
...
def commercial_opportunity_leaderboard_chart(opportunities: pd.DataFrame) -> go.Figure:
    ...
    chart_data = _fixture_labels(opportunities).head(12)
    chart_data = chart_data.sort_values("commercial_opportunity_score", ascending=True)
    fig = px.bar(
        chart_data,
    ...
    _highlight_leader(fig, len(chart_data), BLUE)
...
def venue_capacity_utilisation_chart(opportunities: pd.DataFrame) -> go.Figure:
    ...
    chart_data = chart_data.sort_values(
        "estimated_capacity_utilisation", ascending=True
    )
    fig = px.bar(
        chart_data,
...
def average_crowd_by_venue_chart(opportunities: pd.DataFrame) -> go.Figure:
    ...
    chart_data = chart_data.sort_values("average_crowd", ascending=True)
    fig = px.bar(
        chart_data,
        x="average_crowd",
        y="venue",
...
def fixture_equity_risk_chart(fixture_equity: pd.DataFrame) -> go.Figure:
    ...
    chart_data = fixture_equity.sort_values("fixture_equity_risk_score", ascending=True)
    fig = px.bar(
        chart_data,
...
def short_break_games_chart(fixture_equity: pd.DataFrame) -> go.Figure:
    ...
    chart_data = fixture_equity.sort_values("short_break_games", ascending=True)
    fig = px.bar(
        chart_data,
        x="short_break_games",
        y="team",
    ...
    _highlight_leader(fig, len(chart_data), MUTED_RED)
...
def interstate_away_games_chart(travel_load: pd.DataFrame) -> go.Figure:
    ...
    chart_data = travel_load.sort_values("interstate_away_games", ascending=True)
    fig = px.bar(
        chart_data,
...
def estimated_travel_km_chart(travel_load: pd.DataFrame) -> go.Figure:
    ...
    chart_data = travel_load.sort_values("estimated_travel_km", ascending=True)
    fig = px.bar(
        chart_data,
...
def travel_load_score_chart(travel_load: pd.DataFrame) -> go.Figure:
    ...
    chart_data = travel_load.sort_values("travel_load_score", ascending=True)
    fig = px.bar(
        chart_data,
        x="travel_load_score",
        y="team",
...
def leader_highlight_colours(row_count: int, base_colour: str) -> list[str]:
    """Return colours for ascending bars with the highest value highlighted."""
    if row_count <= 0:
        return []
    return [base_colour] * (row_count - 1) + [GOLD]


def _highlight_leader(fig: go.Figure, row_count: int, base_colour: str) -> None:
    """Apply accessible emphasis to the leading bar in a ranked chart."""
    fig.update_traces(marker_color=leader_highlight_colours(row_count, base_colour))
